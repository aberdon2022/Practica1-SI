...
df_contactos_empleado = df[df['tipo_incidencia'] == 'Fraude'].groupby('empleado').agg({'ticket_id': 'count'}).reset_index().rename(columns={'ticket_id': 'count'})
df_contactos_empleado_por_nivel = df[df['tipo_incidencia'] == 'Fraude'].groupby(['nivel_empleado']).agg({'ticket_id': 'count'}).reset_index().rename(columns={'ticket_id': 'count'})
df_contactos_cliente = df[df['tipo_incidencia'] == 'Fraude'].groupby(['cliente']).agg({'ticket_id': 'count'}).reset_index().rename(columns={'ticket_id': 'count'})
df_contactos_por_dia = df[df['tipo_incidencia'] == 'Fraude'].groupby(['dia']).agg({'ticket_id': 'count'}).reset_index().rename(columns={'ticket_id': 'count'})
